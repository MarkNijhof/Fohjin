using System;
using Fohjin.DDD.EventStore.Bus;
using Fohjin.DDD.EventStore.Storage;
using Fohjin.DDD.EventStore.Storage.Memento;

namespace Fohjin.DDD.EventStore
{
    public class DomainRepository : IDomainRepository
    {
        private readonly IEventStoreUnitOfWork _eventStoreUnitOfWork;
        private readonly IEventHandlerUnitOfWork _eventHandlerUnitOfWork;
        private readonly IIdentityMap _identityMap;

        public DomainRepository(IEventStoreUnitOfWork eventStoreUnitOfWork, IEventHandlerUnitOfWork eventHandlerUnitOfWork, IIdentityMap identityMap)
        {
            _eventStoreUnitOfWork = eventStoreUnitOfWork;
            _eventHandlerUnitOfWork = eventHandlerUnitOfWork;
            _identityMap = identityMap;
        }

        public TAggregate GetById<TAggregate>(Guid id) where TAggregate : class, IOrginator, IEventProvider, new()
        {
            var aggregateRoot = RegisterForTracking(_identityMap.GetById<TAggregate>(id)) ?? _eventStoreUnitOfWork.GetById<TAggregate>(id);
            return RegisterForPublishing(aggregateRoot);
        }

        public void Add<TAggregate>(TAggregate aggregateRoot) where TAggregate : class, IOrginator, IEventProvider, new()
        {
            _eventStoreUnitOfWork.Add(aggregateRoot);
            _eventHandlerUnitOfWork.RegisterForPublishing(aggregateRoot);
        }

        public void Complete()
        {
            try
            {
                _eventHandlerUnitOfWork.Complete();
                _eventStoreUnitOfWork.Complete();

                _eventHandlerUnitOfWork.Commit();
                _eventStoreUnitOfWork.Commit();
            }
            catch (Exception Ex)
            {
                _eventHandlerUnitOfWork.Rollback();
                _eventStoreUnitOfWork.Rollback();
                throw;
            }
        }

        private TAggregate RegisterForTracking<TAggregate>(TAggregate aggregateRoot) where TAggregate : class, IOrginator, IEventProvider, new()
        {
            if (aggregateRoot == null)
                return aggregateRoot;

            _eventStoreUnitOfWork.RegisterForTracking(aggregateRoot);
            return aggregateRoot;
        }

        private TAggregate RegisterForPublishing<TAggregate>(TAggregate aggregateRoot) where TAggregate : class, IOrginator, IEventProvider, new()
        {
            if (aggregateRoot == null)
                return aggregateRoot;

            _eventHandlerUnitOfWork.RegisterForPublishing(aggregateRoot);
            return aggregateRoot;
        }
    }
}